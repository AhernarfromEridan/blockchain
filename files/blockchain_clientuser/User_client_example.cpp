...
void SendTransactionToServer(std::string transaction, SOCKET serverSocket) {
	...
	std::cout << "Sent out transaction data to the server\n";
}
...
std::string HandleIncomingData(std::string data) {
	...
	if (type == "Confirmation") {
		if (data == "True") {
			printf("Received confirmation for transaction sent;\n");
		}
		else {
...
DWORD WINAPI SessionWithServer(LPVOID data) {
	...
	std::cout << "Sent out client type data to the server\n";
...
int main(int argc, char **argv)
{
	...
	//if (ConnectSocket == INVALID_SOCKET) {
	//	cout << "Connection failed, restart the program while making sure server is running\n";
	//	return 1;
	//}
	...
	std::cout << "Public key length:\n" << string_to_hex(encodedPublicKey).length() << std::endl;
	std::cout << "Private key:\n" << string_to_hex(encodedPrivateKey) << std::endl;
	std::cout << "Private key length:\n" << string_to_hex(encodedPrivateKey).length() << std::endl;
